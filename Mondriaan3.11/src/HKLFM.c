...
int CoarsenGraph(struct biparthypergraph *pHG,
    ...
    */
    ...
    int *Matched; /* Matched[v] = TRUE means vertex v has already
                      been matched */
    long *iAdjncy, *NetStart, *NetEnd; /* arrays of length NrNets */
    ...
    if (pOptions->Coarsening_MatchingStrategy == MatchRandom) {
    ...
    if (pOptions->Coarsening_MatchingStrategy == MatchRandom ||
        ...
        NetStart = (long *) malloc(pHG->NrNets * sizeof(long));
        NetEnd = (long *) malloc(pHG->NrNets * sizeof(long));

        if (Ip == NULL || ScIp == NULL || Visited == NULL || NetStart == NULL || NetEnd  == NULL) {
            fprintf(stderr, "CoarsenGraph(): Not enough memory!\n");
            return FALSE;
        }
