...
int remembrance_combine(struct remembrance * const m, struct ordertreerange * const r, const long r_size, const char empty_first) {
    unsigned long range_id, m_id, e_id;

    /*for( m_id = 0; m_id<r_size; m_id++ ) printf( "Ordertree: index=%lu, id=%lu, parent=%lu, meta=%d\n", r[ m_id ].PiLo, r[ m_id ].id, r[ m_id ].parent, r[ m_id ].Meta );
    for( m_id = 0; m_id<m->empty.size; m_id++ ) printf( "Emptycuts: index=%lu, id=%lu, parent=%lu\n", m->empty.vector[ m_id ].index, m->empty.vector[ m_id ].id, m->empty.vector[ m_id ].parent );*/
    ...
    /* Set current indices */
    m_id = range_id = e_id = 0;

    /* Go through all elements in ordertree ranges */
    for( ; range_id < r_size; range_id++ ) {
        unsigned long index;
    ...
    /*for( m_id = 0; m_id<m->size; m_id++ ) printf( "To match : index=%lu, id=%lu, parent=%lu\n", m->vector[ m_id ].index, m->vector[ m_id ].id, m->vector[ m_id ].parent );*/
