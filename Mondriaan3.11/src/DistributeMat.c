...
int DistributeMatrixPaToH(struct sparsematrix *pT, int P, double eps, const struct opts *pOptions) {
    ...
    */
    ...
    for (t = 0; t < pHG->NrVertices; ++t) {
        partvec[t] = 0;
        weighttot += (cwghts[t] = pHG->V[t].vtxwgt);
    }
...
int DistributeMatrixMondriaan(struct sparsematrix *pT, int P, double eps, const struct opts *pOptions, int (*Callback)(int, int, const struct sparsematrix *)) {
    ...
    */
    ...
    while (done == FALSE && k < P) {
        ...
        printf("\n");
#endif
        /* Find cut rows and columns. */
        if (pOptions->Metric == MetricCut) {
            long countrows = 0, countcols = 0;
            
            for (t = pT->Pstart[i]; t < pT->Pstart[i + 1]; ++t) {
                if (pT->RowMark[pT->i[t]] == 0) pT->RowMark[pT->i[t]] = 2;
                if (pT->ColMark[pT->j[t]] == 0) pT->ColMark[pT->j[t]] = 2;
            }
            
            for (t = pT->Pstart[i + 1]; t < pT->Pstart[i + 2]; ++t) {
                if (pT->RowMark[pT->i[t]] == 2) pT->RowMark[pT->i[t]] = 3;
                if (pT->ColMark[pT->j[t]] == 2) pT->ColMark[pT->j[t]] = 3;
            }
            
            for (t = pT->Pstart[i]; t < pT->Pstart[i + 2]; ++t) {
                if (pT->RowMark[pT->i[t]] == 2) pT->RowMark[pT->i[t]] = 0;
                else if (pT->RowMark[pT->i[t]] == 3) {pT->RowMark[pT->i[t]] = 1; ++countrows;}
                if (pT->ColMark[pT->j[t]] == 2) pT->ColMark[pT->j[t]] = 0;
                else if (pT->ColMark[pT->j[t]] == 3) {pT->ColMark[pT->j[t]] = 1; ++countcols;}
            }
            
#ifdef INFO2
            printf("DistributeMatrixMondriaan(): Marked %ld rows and %ld columns.\n", countrows, countcols);
#endif
        }
        
        /* Keep track of the way the collection of nonzeroes is being split and permute matrix. */
        if (pOptions->OrderPermutation != OrderNone) {
            if (!Symmetric) {
