...
int SparseMatrix2BiPartHyperGraph(const struct sparsematrix *pM, 
    ...
    */
    ...
    if (pHG->SplitDir == ROW) {
        ...
        for (j = 0; j < pM->n; j++) {
            if (nnzC[j] > 0) {
                pHG->N[t].netwgt = 1; /* uniform netweights for the time being */
                pHG->N[t].iStartP0 = pHG->N[t].iStartP1 = pHG->N[t].iEnd = pin_index;
                pHG->N[t].dir = COL;
                pHG->N[t].Free = (pM->ColMark[j] == 1 ? TRUE : FALSE);
                pin_index += nnzC[j];
        ...
        for (i = 0; i < pM->m; i++) {
            if (nnzR[i] > 0) {
                ...
                pHG->N[t].iStartP0 = pHG->N[t].iStartP1 = pHG->N[t].iEnd = pin_index;
                pHG->N[t].dir = ROW;
                pHG->N[t].Free = (pM->RowMark[i] == 1 ? TRUE : FALSE);
                pin_index += nnzR[i];
        ...
        /* Count number of nonzeroes (pins) in each net */
        for (i = 0; i < pM->n; i++)
            Col2HGIndex[i] = 0;
        for (t = 0; t < pM->NrNzElts; t++) {
            if( pM->i[t] == pM->j[t] )
        ...
        for (i = 0; i < pM->m; i++) {
            if (nnzR[i] > 0 || nnzC[i]>0) {
                pHG->N[t].netwgt = 2; /* uniform, 2 since each row net also represents a column */
                pHG->N[t].iStartP0 = pHG->N[t].iStartP1 = pHG->N[t].iEnd = pin_index;
                pHG->N[t].dir = ROW;
                pHG->N[t].Free = (pM->RowMark[i] == 1 ? TRUE : FALSE);
...
int BiPartHyperGraph2SparseMatrix(const struct biparthypergraph *pHG, long lo, long hi, long *mid, struct sparsematrix *pM) {
    ...
    /* This function translates a hypergraph HG into a subset of the nonzeros
       ...
       The remainder of T remains unchanged.
    ...
    */
    ...
    for (t = 0; t < pHG->NrVertices; t++) {
        ...
        if (pHG->SplitDir == ROW || pHG->SplitDir == COL) {
            ...
            /* Create one or two new nonzeroes for the first net adjacent to vertex t */
