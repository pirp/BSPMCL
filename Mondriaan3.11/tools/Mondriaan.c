 ...
 */
    ...
    long int **row_perms, **col_perms; /* Will store local to global index info */
    ...
    if (Options.OutputMode == MultipleFiles) {
        ...
        /* Write the vector distribution to file */
        WriteVectorDistribution(v_proc, "Input-vector", A.m, A.NrProcs, File, &Options);
  
        WriteVectorDistribution(u_proc, "Output-vector", A.n, A.NrProcs, File, &Options);
